# Geo5017ML_Assignment1
Group 10: Job Segers, Timber Groeneveld, Akhil Veeranki 

# Operation of the code
The main.py file is the only file that needs to be ran. 
Data is directly written in the 'main' function of the file. Parameters for the gradient descent solver are at the top of the file.
Models.py contains the data model classes, with methods to return the position at time t and to give the gradient given a set of data and their current parameter values.
drone_tracking_2.1_plot.py is an independent file used for the exercise 2.1 plot. 
